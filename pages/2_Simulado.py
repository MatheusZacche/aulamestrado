"""Modo Simulado — 20 questões, 2h, sem feedback até o fim.

Reproduz o formato da prova oficial PPGI:
- 20 questões objetivas
- 5 alternativas, 1 correta
- 2 horas de duração
- Mínimo 60% para aprovação (eliminatório)
"""
...
def gerar_simulado(seed: int | None = None) -> list[Question]:
    ...
    for e in eixos:
        ...
        rnd.shuffle(pool_sorted[:30])  # embaralha leve dentro do topo
        selecionadas.extend(pool_sorted[:5])

    # Se algum eixo está vazio, completa com outras
...
if not st.session_state.sim_iniciado:
    st.info(
        f"Banco atual: **{len(bank.questions)}** questões. "
        "O simulado escolhe 20, distribuídas pelos 4 eixos do edital."
    )
    seed = st.text_input("Seed opcional (mesma seed = mesmo simulado, útil pra refazer)", value="")
...
if st.session_state.get("sim_finalizado"):
    ...
    c3.metric("Status", "✅ APROVADO" if aprovado else "❌ ELIMINADO",
    ...
    if st.button("Novo simulado", type="primary"):
...
# Layout: navegação por questão (botões numerados) + conteúdo
nav_cols = st.columns(20)
if "sim_idx" not in st.session_state:
    st.session_state.sim_idx = 0
for i in range(20):
    q = questoes[i]
    respondida = q.id in respostas
    label = f"{'✓' if respondida else '·'}{i + 1}"
    if nav_cols[i].button(label, key=f"nav_q_{i}"):
...
if cprev.button("⬅️ Anterior", disabled=idx == 0):
    st.session_state.sim_idx = max(0, idx - 1)
    st.rerun()
if cnext.button("Próxima ➡️", disabled=idx == 19):
    st.session_state.sim_idx = min(19, idx + 1)
    st.rerun()

# auto-refresh do cronômetro a cada 30s
time.sleep(0)  # placeholder; refresh acontece via interação do usuário
