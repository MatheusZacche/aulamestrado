...
# =========================================================================
# LOTE C3 — Linguagens e Paradigmas (+20 questões: ids 0048-0067)
# =========================================================================
LOTE_C3 = [
    _q(
        48, "paradigmas", ["paradigma_imperativo", "paradigma_funcional"], "facil",
        "Qual a diferença ESSENCIAL entre os paradigmas imperativo e declarativo?",
        [
            ("a", "Imperativo é mais rápido que declarativo.",
             "ERRADA. Velocidade depende da implementação e do problema; não é diferença essencial."),
            ("b", "Imperativo descreve COMO realizar a tarefa (sequência de passos e mutação de estado); declarativo descreve O QUE se deseja como resultado.",
             "CORRETA. Diferença conceitual fundamental. Imperativo: 'faça x = x+1, depois...'. Declarativo: 'a função soma é definida como...'."),
            ("c", "Imperativo não suporta funções; declarativo sim.",
             "ERRADA. Imperativo (C, Java) suporta funções normalmente."),
            ("d", "Declarativo só funciona em hardware paralelo.",
             "ERRADA. Funciona em qualquer hardware."),
            ("e", "Imperativo é orientado a objetos; declarativo é funcional.",
             "ERRADA. OO é subtipo de imperativo, mas declarativo engloba mais que funcional (inclui lógico, etc.)."),
        ],
        "b",
    ),
    _q(
        49, "paradigmas", ["paradigma_funcional"], "medio",
        "No paradigma funcional puro, o que significa 'transparência referencial'?",
        [
            ("a", "Variáveis podem mudar de valor durante a execução.",
             "ERRADA. Isso seria mutação, antiético ao paradigma funcional puro."),
            ("b", "Uma expressão pode ser substituída pelo seu valor sem alterar o comportamento do programa.",
             "CORRETA. Transparência referencial é a propriedade de que expressões PURAS (sem efeitos colaterais e que dependem apenas de seus argumentos) podem ser substituídas por seus valores. Permite raciocínio equacional e otimizações como memoização."),
            ("c", "Toda função tem efeitos colaterais.",
             "ERRADA. Pelo contrário: ausência de efeitos colaterais é pré-requisito."),
            ("d", "Funções devem usar referências em vez de valores.",
             "ERRADA. Confusão com referências/ponteiros."),
            ("e", "Variáveis devem ser passadas por referência.",
             "ERRADA. Mesmo erro de (d)."),
        ],
        "b",
    ),
    _q(
        50, "paradigmas", ["paradigma_logico"], "medio",
        "Qual característica é PRÓPRIA do paradigma de programação lógica (ex: Prolog)?",
        [
            ("a", "Uso de classes e herança como base de organização.",
             "ERRADA. Isso é orientação a objetos."),
            ("b", "Definição do programa como conjunto de fatos e regras, com inferência via unificação e backtracking.",
             "CORRETA. Programação lógica representa conhecimento como fatos e regras; a execução é uma busca por provas (resolução) usando unificação."),
            ("c", "Sequência explícita de comandos atribuindo valores a variáveis.",
             "ERRADA. Característica do paradigma imperativo."),
            ("d", "Funções de primeira ordem e ausência de mutação.",
             "ERRADA. Característica do paradigma funcional."),
            ("e", "Tipos algébricos e pattern matching exaustivo.",
             "ERRADA. Característica do paradigma funcional tipado (Haskell, ML)."),
        ],
        "b",
    ),
    _q(
        51, "paradigmas", ["principios_solid"], "medio",
        "O Princípio Aberto-Fechado (Open/Closed Principle - OCP) estabelece que:",
        [
            ("a", "Toda classe deve ser pública (aberta) e seus métodos privados (fechados).",
             "ERRADA. Não tem relação com modificadores de acesso."),
            ("b", "Entidades de software (classes, módulos) devem estar ABERTAS para extensão, mas FECHADAS para modificação.",
             "CORRETA. Você deve poder adicionar comportamentos sem alterar o código existente. Tipicamente alcançado via polimorfismo: adiciona-se subclasse com novo comportamento em vez de modificar a base."),
            ("c", "Interfaces devem ser pequenas e específicas.",
             "ERRADA. Esse é o Interface Segregation Principle (ISP)."),
            ("d", "Cada classe deve ter uma única responsabilidade.",
             "ERRADA. Esse é o Single Responsibility Principle (SRP)."),
            ("e", "Módulos não devem depender de implementações concretas.",
             "ERRADA. Esse é o Dependency Inversion Principle (DIP)."),
        ],
        "b",
    ),
    _q(
        52, "paradigmas", ["paradigma_oo", "encapsulamento_abstracao"], "facil",
        "Qual a diferença entre ABSTRAÇÃO e ENCAPSULAMENTO em POO?",
        [
            ("a", "São sinônimos.",
             "ERRADA. São conceitos relacionados mas distintos."),
            ("b", "Abstração esconde a complexidade mostrando apenas o essencial (foca no 'O QUE'); encapsulamento protege o estado interno controlando o acesso (foca no 'COMO esconder').",
             "CORRETA. Abstração é uma decisão de design (que detalhes mostrar). Encapsulamento é o mecanismo (modificadores de acesso, getters/setters) que efetiva essa abstração."),
            ("c", "Abstração é exclusiva de Java; encapsulamento de C++.",
             "ERRADA. Ambos são conceitos universais em POO."),
            ("d", "Abstração permite herança múltipla; encapsulamento não.",
             "ERRADA. Sem relação direta com herança múltipla."),
            ("e", "Abstração é mais lenta que encapsulamento.",
             "ERRADA. Performance não é o critério."),
        ],
        "b",
    ),
    _q(
        53, "paradigmas", ["heranca_polimorfismo"], "medio",
        "Em POO, qual a diferença entre OVERLOAD (sobrecarga) e OVERRIDE (sobrescrita)?",
        [
            ("a", "Overload é em compilação; override é em runtime.",
             "ERRADA. Parcialmente verdade mas insuficiente; falta a essência."),
            ("b", "Overload: mesma classe, métodos com MESMO NOME e DIFERENTES ASSINATURAS (parâmetros). Override: subclasse REDEFINE método herdado da superclasse com MESMA ASSINATURA.",
             "CORRETA. Overload é polimorfismo ad-hoc (resolvido em compilação por assinatura). Override é polimorfismo de subtipo (resolvido em runtime pela dispatch dinâmica)."),
            ("c", "Overload requer herança; override não.",
             "ERRADA. Overload acontece na mesma classe — NÃO requer herança."),
            ("d", "São sinônimos em linguagens modernas.",
             "ERRADA. Sempre distintos."),
            ("e", "Overload usa interfaces; override usa classes abstratas.",
             "ERRADA. Não há relação fixa com esses mecanismos."),
        ],
        "b",
    ),
    _q(
        54, "paradigmas", ["tipagem_estatica_dinamica"], "medio",
        "Sobre tipagem em linguagens de programação, qual afirmação é CORRETA?",
        [
            ("a", "Tipagem forte significa tipagem estática.",
             "ERRADA. Forte/fraca refere-se a conversões; estática/dinâmica a quando os tipos são verificados. Python é DINAMICAMENTE FORTE; C é ESTATICAMENTE FRACO."),
            ("b", "Python é fortemente tipada e dinamicamente tipada.",
             "CORRETA. Python é DINÂMICA (tipos verificados em runtime, variável pode mudar de tipo) e FORTE (operações entre tipos incompatíveis levantam erro, ex: 'a' + 1 lança TypeError)."),
            ("c", "JavaScript é fortemente tipada.",
             "ERRADA. JS é fracamente tipada (conversões implícitas, ex: '5' + 1 vira '51')."),
            ("d", "C++ é dinamicamente tipada.",
             "ERRADA. C++ é estaticamente tipada (tipos no momento da compilação)."),
            ("e", "Tipagem dinâmica e tipagem fraca são equivalentes.",
             "ERRADA. São dimensões ortogonais."),
        ],
        "b",
    ),
    _q(
        55, "paradigmas", ["paradigma_funcional"], "facil",
        "Qual operação é típica do paradigma funcional?",
        [
            ("a", "for (i=0; i<n; i++) { v[i] = v[i]*2; }",
             "ERRADA. Loop imperativo com mutação — antiético ao funcional."),
            ("b", "v.forEach(item => print(item))",
             "ERRADA. Funcional mas há efeito colateral (impressão)."),
            ("c", "map(lambda x: x*2, lista)",
             "CORRETA. map() é uma função de ordem superior clássica do paradigma funcional: aplica uma função a cada elemento, retornando nova coleção sem mutar a original."),
            ("d", "stack.push(elemento)",
             "ERRADA. Mutação de estado — paradigma imperativo/OO."),
            ("e", "x = x + 1",
             "ERRADA. Reatribuição (mutação) — não funcional."),
        ],
        "c",
    ),
    _q(
        56, "paradigmas", ["principios_solid"], "dificil",
        ...
        [
            ("a", "Módulos de alto nível NÃO devem depender de módulos de baixo nível; ambos devem depender de ABSTRAÇÕES.",
             "CORRETA. Inversão de dependência: detalhes (módulos de baixo nível) dependem de abstrações (interfaces), e o código de alto nível também depende dessas abstrações — não diretamente da implementação."),
            ("b", "Subclasses devem poder substituir superclasses.",
             "ERRADA. Esse é o Liskov Substitution Principle (LSP)."),
            ("c", "Uma classe deve ter apenas uma razão para mudar.",
             "ERRADA. Esse é o Single Responsibility Principle (SRP)."),
            ("d", "Classes devem ser abertas para extensão, fechadas para modificação.",
             "ERRADA. Esse é o Open/Closed Principle (OCP)."),
            ("e", "Clientes não devem ser forçados a depender de métodos que não usam.",
             "ERRADA. Esse é o Interface Segregation Principle (ISP)."),
        ],
        "a",
    ),
    _q(
        57, "paradigmas", ["paradigma_oo", "heranca_polimorfismo"], "medio",
        "Em POO, a composição (composition) é frequentemente preferida à herança porque:",
        [
            ("a", "Composição é mais rápida em runtime.",
             "ERRADA. Performance é parecida; não é o critério."),
            ("b", "Composição evita o forte acoplamento da hierarquia de herança e permite mudar o comportamento em tempo de execução, seguindo o princípio 'favor composition over inheritance'.",
             "CORRETA. Herança cria acoplamento rígido na hierarquia (mudanças na superclasse propagam). Composição usa instâncias internas, permitindo flexibilidade e mudança dinâmica de comportamento (ex: design pattern Strategy)."),
            ("c", "Herança não funciona em linguagens modernas.",
             "ERRADA. Herança continua funcionando em Java, C++, Python, etc."),
            ("d", "Composição é a única forma de reutilizar código.",
             "ERRADA. Herança também reutiliza código."),
            ("e", "Composição é exigida pelo padrão SOLID.",
             "ERRADA. SOLID não exige composição; recomenda design flexível."),
        ],
        "b",
    ),
    _q(
        58, "paradigmas", ["paradigma_oo", "encapsulamento_abstracao"], "facil",
        "Qual modificador de acesso em Java permite acesso apenas pelas classes do mesmo PACKAGE (e exclui subclasses fora do package)?",
        [
            ("a", "public",
             "ERRADA. public permite acesso de qualquer lugar."),
            ("b", "private",
             "ERRADA. private só permite acesso na própria classe."),
            ("c", "protected",
             "ERRADA. protected permite acesso pelo package E por subclasses (mesmo fora do package)."),
            ("d", "(padrão / package-private / default — sem modificador)",
             "CORRETA. Em Java, ausência de modificador (chamado 'default' ou 'package-private') torna visível apenas dentro do mesmo package. Subclasses em outros packages NÃO têm acesso."),
            ("e", "static",
             "ERRADA. static não é modificador de acesso, mas de tempo de vida/escopo de classe."),
        ],
        "d",
    ),
    _q(
        59, "paradigmas", ["paradigma_imperativo"], "facil",
        "Qual destas linguagens é PRIMARIAMENTE imperativa (não funcional, não puramente OO)?",
        [
            ("a", "Haskell",
             "ERRADA. Haskell é funcional pura."),
            ("b", "Prolog",
             "ERRADA. Prolog é lógica."),
            ("c", "C",
             "CORRETA. C é a linguagem imperativa procedural por excelência: estado mutável, sequência explícita de instruções, sem orientação a objetos nativa, sem mecanismos funcionais avançados."),
            ("d", "Smalltalk",
             "ERRADA. Smalltalk é uma das mais puras OO."),
            ("e", "Erlang",
             "ERRADA. Erlang é funcional concorrente."),
        ],
        "c",
    ),
    _q(
        60, "paradigmas", ["heranca_polimorfismo"], "medio",
        "Qual tipo de polimorfismo é exemplificado por templates em C++ e generics em Java?",
        [
            ("a", "Polimorfismo de subtipo (subtype polymorphism)",
             "ERRADA. Subtipo é via herança e dispatch dinâmico (override)."),
            ("b", "Polimorfismo paramétrico (parametric polymorphism)",
             "CORRETA. Templates/generics permitem que o mesmo código opere sobre múltiplos tipos parametricamente, sem perda de segurança de tipos. Ex: List<T> em Java pode ser List<Integer> ou List<String>."),
            ("c", "Polimorfismo ad-hoc (overload)",
             "ERRADA. Ad-hoc é sobrecarga de métodos com mesmo nome mas implementações diferentes para cada tipo."),
            ("d", "Polimorfismo de coerção",
             "ERRADA. Coerção é conversão automática de tipos (ex: int → float)."),
            ("e", "Não é polimorfismo, é metaprogramação.",
             "ERRADA. É polimorfismo paramétrico, embora templates C++ também ofereçam metaprogramação."),
        ],
        "b",
    ),
    _q(
        61, "paradigmas", ["paradigma_oo"], "facil",
        "Qual é a unidade fundamental do paradigma orientado a objetos?",
        [
            ("a", "Função",
             "ERRADA. Função é unidade do paradigma funcional."),
            ("b", "Procedimento",
             "ERRADA. Procedimento é unidade do paradigma procedural."),
            ("c", "Objeto (instância de classe)",
             "CORRETA. Em POO, programas são modelados como interações entre objetos, cada um encapsulando estado e comportamento. Classes são moldes; objetos são as unidades operacionais."),
            ("d", "Fato e regra",
             "ERRADA. Fatos e regras são unidades do paradigma lógico."),
            ("e", "Variável",
             "ERRADA. Variáveis são unidades mais primitivas, não característica definidora de POO."),
        ],
        "c",
    ),
    _q(
        62, "paradigmas", ["principios_solid"], "medio",
        "O Princípio da Responsabilidade Única (SRP) NÃO significa que:",
        [
            ("a", "Uma classe deve ter apenas uma razão para mudar.",
             "ERRADA (afirmação é CORRETA conforme SRP). Não é a resposta pedida."),
            ("b", "Classes maiores que 200 linhas violam SRP.",
             "CORRETA (essa é a NÃO significa). SRP não dita tamanho de classes; é sobre coesão e responsabilidade conceitual. Uma classe pode ter 500 linhas e ter uma única responsabilidade (e vice-versa)."),
            ("c", "Diferentes responsabilidades devem ser separadas em classes distintas.",
             "ERRADA (afirmação é correta conforme SRP)."),
            ("d", "Uma classe deve focar em um único aspecto do domínio.",
             "ERRADA (afirmação é correta conforme SRP)."),
            ("e", "Mudanças em diferentes áreas do sistema não devem afetar a mesma classe.",
             "ERRADA (afirmação é correta conforme SRP)."),
        ],
        "b",
    ),
    _q(
        63, "paradigmas", ["tipagem_estatica_dinamica"], "facil",
        "Em uma linguagem com TYPE INFERENCE (inferência de tipos, ex: Haskell, TypeScript, Rust):",
        [
            ("a", "O programador deve sempre declarar o tipo de cada variável.",
             "ERRADA. O ponto da inferência é JUSTAMENTE evitar declarações redundantes."),
            ("b", "Não há verificação de tipos.",
             "ERRADA. Há verificação; o compilador apenas DEDUZ o tipo automaticamente."),
            ("c", "O compilador deduz automaticamente o tipo da maioria das expressões a partir do contexto.",
             "CORRETA. Inferência de tipos analisa o uso e dedução algébrica (Hindley-Milner em Haskell, por exemplo) para atribuir tipos sem declaração explícita."),
            ("d", "A linguagem é necessariamente dinamicamente tipada.",
             "ERRADA. Inferência é sobre quem ESCREVE o tipo; pode ser estática."),
            ("e", "Não pode haver polimorfismo.",
             "ERRADA. Pelo contrário; inferência funciona muito bem com polimorfismo paramétrico."),
        ],
        "c",
    ),
    _q(
        64, "paradigmas", ["paradigma_oo", "encapsulamento_abstracao"], "medio",
        "Em Java, qual a diferença entre `class abstrata` e `interface`?",
        [
            ("a", "Não há diferença em Java moderno.",
             "ERRADA. Há diferenças importantes mesmo em Java 8+."),
            ("b", "Classe abstrata pode ter estado (atributos), construtores e métodos com implementação. Interface (pré-Java 8) só tinha métodos abstratos; (pós-8) pode ter `default` e `static`, mas não estado e nem construtores. Uma classe estende UMA abstrata mas IMPLEMENTA várias interfaces.",
             "CORRETA. Essas são as diferenças clássicas e práticas."),
            ("c", "Interface é mais rápida que classe abstrata.",
             "ERRADA. Performance é equivalente."),
            ("d", "Classes abstratas não podem ter métodos não-abstratos.",
             "ERRADA. Podem ter métodos concretos."),
            ("e", "Interfaces permitem herança múltipla de implementação.",
             "ERRADA. Métodos default em interfaces evitam apenas obrigatoriedade de implementação, mas Java mitiga diamond com regras específicas."),
        ],
        "b",
    ),
    _q(
        65, "paradigmas", ["paradigma_funcional"], "medio",
        "Em programação funcional, o que é uma 'função pura'?",
        [
            ("a", "Uma função escrita em uma linguagem funcional como Haskell.",
             "ERRADA. Pureza não depende da linguagem, mas das propriedades da função."),
            ("b", "Uma função que, dado o mesmo input, sempre retorna o mesmo output e não causa efeitos colaterais observáveis.",
             "CORRETA. Pureza tem dois aspectos: determinismo (mesmo input → mesmo output) e ausência de efeitos colaterais (não modifica estado externo, não faz I/O, etc.)."),
            ("c", "Uma função que não tem parâmetros.",
             "ERRADA. Sem parâmetros = constante; pureza é independente."),
            ("d", "Uma função que retorna apenas valores numéricos.",
             "ERRADA. Pureza não depende do tipo do retorno."),
            ("e", "Uma função recursiva sem laço.",
             "ERRADA. Pureza independe de recursão."),
        ],
        "b",
    ),
    _q(
        66, "paradigmas", ["principios_solid"], "dificil",
        "Qual cenário VIOLA o Princípio da Segregação de Interfaces (ISP)?",
        [
            ("a", "Uma interface `Imprimivel` com um único método `imprimir()`.",
             "ERRADA. Interface focada — conforme ISP."),
            ("b", "Uma interface `Worker` com métodos `trabalhar()`, `comer()` e `dormir()`, implementada por classes `Humano` e `Robo` — onde `Robo` precisa implementar `comer()` e `dormir()` mesmo que sejam métodos sem sentido para ele.",
             "CORRETA. ISP afirma que clientes não devem ser forçados a depender de métodos que não usam. `Robo` ser obrigado a implementar `comer()` viola o princípio. Solução: dividir em `Worker`, `Eater`, `Sleeper`."),
            ("c", "Duas interfaces separadas: `Leitor` (com `ler()`) e `Escritor` (com `escrever()`).",
             "ERRADA. Boa aplicação de ISP."),
            ("d", "Uma classe que implementa três interfaces relacionadas mas distintas.",
             "ERRADA. Múltiplas interfaces pequenas estão alinhadas com ISP."),
            ("e", "Uma interface que herda de outra interface menor.",
             "ERRADA. Hierarquia de interfaces não viola ISP por si."),
        ],
        "b",
    ),
    _q(
        67, "paradigmas", ["paradigma_oo"], "facil",
        "Qual mecanismo permite que uma única referência possa apontar para objetos de diferentes classes relacionadas por herança, executando o método correto em runtime?",
        [
            ("a", "Encapsulamento",
             "ERRADA. Encapsulamento é sobre proteção de estado, não dispatch dinâmico."),
            ("b", "Herança",
             "ERRADA. Herança permite a relação de tipos, mas não o dispatch dinâmico per se."),
            ("c", "Polimorfismo (dispatch dinâmico via override)",
             "CORRETA. Em linguagens como Java/C++, métodos virtuais (sobrescritos) são resolvidos em runtime baseados no tipo REAL do objeto, não no tipo declarado da referência. Isso é o polimorfismo de subtipo."),
            ("d", "Abstração",
             "ERRADA. Abstração é decisão de design, não mecanismo de runtime."),
            ("e", "Composição",
             "ERRADA. Composição é alternativa à herança, não relacionada a dispatch dinâmico."),
        ],
        "c",
    ),
]
